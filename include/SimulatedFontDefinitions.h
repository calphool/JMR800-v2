#pragma once

#ifndef TARGET_TEENSY

#include <cstdint>

const uint8_t defaultFont[128][5] = {
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   0 'NUL'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   1 'SOH'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   2 'STX'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   3 'ETX'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   4 'EOT'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   5 'ENQ'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   6 'ACK'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   7 'BEL'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   8 'BS'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //   9 'TAB'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  10 'LF'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  11 'VT'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  12 'FF'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  13 'CR'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  14 'SO'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  15 'SI'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  16 'DLE'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  17 'DC1'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  18 'DC2'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  19 'DC3'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  20 'DC4'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  21 'NAK'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  22 'SYN'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  23 'ETB'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  24 'CAN'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  25 'EM'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  26 'SUB'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  27 'ESC'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  28 'FS'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  29 'GS'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  30 'RS'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  31 'US'
    {0x00, 0x00, 0x00, 0x00, 0x00}, //  32 ' '
    {0x00, 0x00, 0x5F, 0x00, 0x00}, //  33 '!'
    {0x00, 0x07, 0x00, 0x07, 0x00}, //  34 '"'
    {0x14, 0x7F, 0x14, 0x7F, 0x14}, //  35 '#'
    {0x24, 0x2A, 0x7F, 0x2A, 0x12}, //  36 '$'
    {0x23, 0x13, 0x08, 0x64, 0x62}, //  37 '%'
    {0x36, 0x49, 0x55, 0x22, 0x50}, //  38 '&'
    {0x00, 0x05, 0x03, 0x00, 0x00}, //  39 '''
    {0x00, 0x1C, 0x22, 0x41, 0x00}, //  40 '('
    {0x00, 0x41, 0x22, 0x1C, 0x00}, //  41 ')'
    {0x14, 0x08, 0x3E, 0x08, 0x14}, //  42 '*'
    {0x08, 0x08, 0x3E, 0x08, 0x08}, //  43 '+'
    {0x00, 0x50, 0x30, 0x00, 0x00}, //  44 ','
    {0x08, 0x08, 0x08, 0x08, 0x08}, //  45 '-'
    {0x00, 0x60, 0x60, 0x00, 0x00}, //  46 '.'
    {0x20, 0x10, 0x08, 0x04, 0x02}, //  47 '/'
    {0x3E, 0x51, 0x49, 0x45, 0x3E}, //  48 '0'
    {0x00, 0x42, 0x7F, 0x40, 0x00}, //  49 '1'
    {0x72, 0x49, 0x49, 0x49, 0x46}, //  50 '2'
    {0x21, 0x41, 0x49, 0x4D, 0x33}, //  51 '3'
    {0x18, 0x14, 0x12, 0x7F, 0x10}, //  52 '4'
    {0x27, 0x45, 0x45, 0x45, 0x39}, //  53 '5'
    {0x3C, 0x4A, 0x49, 0x49, 0x30}, //  54 '6'
    {0x01, 0x71, 0x09, 0x05, 0x03}, //  55 '7'
    {0x36, 0x49, 0x49, 0x49, 0x36}, //  56 '8'
    {0x06, 0x49, 0x49, 0x29, 0x1E}, //  57 '9'
    {0x00, 0x36, 0x36, 0x00, 0x00}, //  58 ':'
    {0x00, 0x56, 0x36, 0x00, 0x00}, //  59 ';'
    {0x08, 0x14, 0x22, 0x41, 0x00}, //  60 '<'
    {0x14, 0x14, 0x14, 0x14, 0x14}, //  61 '='
    {0x00, 0x41, 0x22, 0x14, 0x08}, //  62 '>'
    {0x02, 0x01, 0x59, 0x09, 0x06}, //  63 '?'
    {0x3E, 0x41, 0x5D, 0x55, 0x1E}, //  64 '@'
    {0x7E, 0x11, 0x11, 0x11, 0x7E}, //  65 'A'
    {0x7F, 0x49, 0x49, 0x49, 0x36}, //  66 'B'
    {0x3E, 0x41, 0x41, 0x41, 0x22}, //  67 'C'
    {0x7F, 0x41, 0x41, 0x22, 0x1C}, //  68 'D'
    {0x7F, 0x49, 0x49, 0x49, 0x41}, //  69 'E'
    {0x7F, 0x09, 0x09, 0x09, 0x01}, //  70 'F'
    {0x3E, 0x41, 0x49, 0x49, 0x7A}, //  71 'G'
    {0x7F, 0x08, 0x08, 0x08, 0x7F}, //  72 'H'
    {0x00, 0x41, 0x7F, 0x41, 0x00}, //  73 'I'
    {0x20, 0x40, 0x41, 0x3F, 0x01}, //  74 'J'
    {0x7F, 0x08, 0x14, 0x22, 0x41}, //  75 'K'
    {0x7F, 0x40, 0x40, 0x40, 0x40}, //  76 'L'
    {0x7F, 0x02, 0x0C, 0x02, 0x7F}, //  77 'M'
    {0x7F, 0x04, 0x08, 0x10, 0x7F}, //  78 'N'
    {0x3E, 0x41, 0x41, 0x41, 0x3E}, //  79 'O'
    {0x7F, 0x09, 0x09, 0x09, 0x06}, //  80 'P'
    {0x3E, 0x41, 0x51, 0x21, 0x5E}, //  81 'Q'
    {0x7F, 0x09, 0x19, 0x29, 0x46}, //  82 'R'
    {0x46, 0x49, 0x49, 0x49, 0x31}, //  83 'S'
    {0x01, 0x01, 0x7F, 0x01, 0x01}, //  84 'T'
    {0x3F, 0x40, 0x40, 0x40, 0x3F}, //  85 'U'
    {0x1F, 0x20, 0x40, 0x20, 0x1F}, //  86 'V'
    {0x3F, 0x40, 0x38, 0x40, 0x3F}, //  87 'W'
    {0x63, 0x14, 0x08, 0x14, 0x63}, //  88 'X'
    {0x07, 0x08, 0x70, 0x08, 0x07}, //  89 'Y'
    {0x61, 0x51, 0x49, 0x45, 0x43}, //  90 'Z'
    {0x00, 0x7F, 0x41, 0x41, 0x00}, //  91 '['
    {0x02, 0x04, 0x08, 0x10, 0x20}, //  92 '\'
    {0x00, 0x41, 0x41, 0x7F, 0x00}, //  93 ']'
    {0x04, 0x02, 0x01, 0x02, 0x04}, //  94 '^'
    {0x40, 0x40, 0x40, 0x40, 0x40}, //  95 '_'
    {0x00, 0x03, 0x07, 0x00, 0x00}, //  96 '`'
    {0x20, 0x54, 0x54, 0x54, 0x78}, //  97 'a'
    {0x7F, 0x48, 0x44, 0x44, 0x38}, //  98 'b'
    {0x38, 0x44, 0x44, 0x44, 0x20}, //  99 'c'
    {0x38, 0x44, 0x44, 0x48, 0x7F}, // 100 'd'
    {0x38, 0x54, 0x54, 0x54, 0x18}, // 101 'e'
    {0x08, 0x7E, 0x09, 0x01, 0x02}, // 102 'f'
    {0x0C, 0x52, 0x52, 0x52, 0x3E}, // 103 'g'
    {0x7F, 0x08, 0x04, 0x04, 0x78}, // 104 'h'
    {0x00, 0x44, 0x7D, 0x40, 0x00}, // 105 'i'
    {0x20, 0x40, 0x44, 0x3D, 0x00}, // 106 'j'
    {0x7F, 0x10, 0x28, 0x44, 0x00}, // 107 'k'
    {0x00, 0x41, 0x7F, 0x40, 0x00}, // 108 'l'
    {0x7C, 0x04, 0x18, 0x04, 0x78}, // 109 'm'
    {0x7C, 0x08, 0x04, 0x04, 0x78}, // 110 'n'
    {0x38, 0x44, 0x44, 0x44, 0x38}, // 111 'o'
    {0x7C, 0x14, 0x14, 0x14, 0x08}, // 112 'p'
    {0x08, 0x14, 0x14, 0x18, 0x7C}, // 113 'q'
    {0x7C, 0x08, 0x04, 0x04, 0x08}, // 114 'r'
    {0x48, 0x54, 0x54, 0x54, 0x20}, // 115 's'
    {0x04, 0x3F, 0x44, 0x40, 0x20}, // 116 't'
    {0x3C, 0x40, 0x40, 0x20, 0x7C}, // 117 'u'
    {0x1C, 0x20, 0x40, 0x20, 0x1C}, // 118 'v'
    {0x3C, 0x40, 0x30, 0x40, 0x3C}, // 119 'w'
    {0x44, 0x28, 0x10, 0x28, 0x44}, // 120 'x'
    {0x0C, 0x50, 0x50, 0x50, 0x3C}, // 121 'y'
    {0x44, 0x64, 0x54, 0x4C, 0x44}, // 122 'z'
    {0x00, 0x08, 0x36, 0x41, 0x00}, // 123 '{'
    {0x00, 0x00, 0x7F, 0x00, 0x00}, // 124 '|'
    {0x00, 0x41, 0x36, 0x08, 0x00}, // 125 '}'
    {0x02, 0x01, 0x02, 0x04, 0x02}, // 126 '~'
    {0x00, 0x00, 0x00, 0x00, 0x00}, // 127 'DEL'
};


const uint8_t smallFont[128][3] = {
    {0x00, 0x00, 0x00}, //   0 'NUL'
    {0x00, 0x00, 0x00}, //   1 'SOH'
    {0x00, 0x00, 0x00}, //   2 'STX'
    {0x00, 0x00, 0x00}, //   3 'ETX'
    {0x00, 0x00, 0x00}, //   4 'EOT'
    {0x00, 0x00, 0x00}, //   5 'ENQ'
    {0x00, 0x00, 0x00}, //   6 'ACK'
    {0x00, 0x00, 0x00}, //   7 'BEL'
    {0x00, 0x00, 0x00}, //   8 'BS'
    {0x00, 0x00, 0x00}, //   9 'TAB'
    {0x00, 0x00, 0x00}, //  10 'LF'
    {0x00, 0x00, 0x00}, //  11 'VT'
    {0x00, 0x00, 0x00}, //  12 'FF'
    {0x00, 0x00, 0x00}, //  13 'CR'
    {0x00, 0x00, 0x00}, //  14 'SO'
    {0x00, 0x00, 0x00}, //  15 'SI'
    {0x00, 0x00, 0x00}, //  16 'DLE'
    {0x00, 0x00, 0x00}, //  17 'DC1'
    {0x00, 0x00, 0x00}, //  18 'DC2'
    {0x00, 0x00, 0x00}, //  19 'DC3'
    {0x00, 0x00, 0x00}, //  20 'DC4'
    {0x00, 0x00, 0x00}, //  21 'NAK'
    {0x00, 0x00, 0x00}, //  22 'SYN'
    {0x00, 0x00, 0x00}, //  23 'ETB'
    {0x00, 0x00, 0x00}, //  24 'CAN'
    {0x00, 0x00, 0x00}, //  25 'EM'
    {0x00, 0x00, 0x00}, //  26 'SUB'
    {0x00, 0x00, 0x00}, //  27 'ESC'
    {0x00, 0x00, 0x00}, //  28 'FS'
    {0x00, 0x00, 0x00}, //  29 'GS'
    {0x00, 0x00, 0x00}, //  30 'RS'
    {0x00, 0x00, 0x00}, //  31 'US'
    {0x00, 0x00, 0x00}, //  32 ' '
    {0x00, 0x17, 0x00}, //  33 '!'
    {0x03, 0x00, 0x03}, //  34 '"'
    {0x1F, 0x0A, 0x1F}, //  35 '#'
    {0x0A, 0x1F, 0x05}, //  36 '$'
    {0x19, 0x04, 0x13}, //  37 '%'
    {0x0A, 0x15, 0x1A}, //  38 '&'
    {0x00, 0x03, 0x00}, //  39 '''
    {0x00, 0x0E, 0x11}, //  40 '('
    {0x11, 0x0E, 0x00}, //  41 ')'
    {0x0A, 0x04, 0x0A}, //  42 '*'
    {0x04, 0x0E, 0x04}, //  43 '+'
    {0x10, 0x08, 0x00}, //  44 ','
    {0x04, 0x04, 0x04}, //  45 '-'
    {0x00, 0x10, 0x00}, //  46 '.'
    {0x18, 0x04, 0x03}, //  47 '/'
    {0x1F, 0x11, 0x1F}, //  48 '0'
    {0x12, 0x1F, 0x10}, //  49 '1'
    {0x1D, 0x15, 0x17}, //  50 '2'
    {0x11, 0x15, 0x1F}, //  51 '3'
    {0x07, 0x04, 0x1F}, //  52 '4'
    {0x17, 0x15, 0x1D}, //  53 '5'
    {0x1F, 0x15, 0x1D}, //  54 '6'
    {0x01, 0x01, 0x1F}, //  55 '7'
    {0x1F, 0x15, 0x1F}, //  56 '8'
    {0x17, 0x15, 0x1F}, //  57 '9'
    {0x00, 0x0A, 0x00}, //  58 ':'
    {0x10, 0x0A, 0x00}, //  59 ';'
    {0x04, 0x0A, 0x11}, //  60 '<'
    {0x0A, 0x0A, 0x0A}, //  61 '='
    {0x11, 0x0A, 0x04}, //  62 '>'
    {0x01, 0x15, 0x07}, //  63 '?'
    {0x0E, 0x15, 0x16}, //  64 '@'
    {0x1F, 0x05, 0x1F}, //  65 'A'
    {0x1F, 0x15, 0x0A}, //  66 'B'
    {0x0E, 0x11, 0x11}, //  67 'C'
    {0x1F, 0x11, 0x0E}, //  68 'D'
    {0x1F, 0x15, 0x11}, //  69 'E'
    {0x1F, 0x05, 0x01}, //  70 'F'
    {0x0E, 0x11, 0x1D}, //  71 'G'
    {0x1F, 0x04, 0x1F}, //  72 'H'
    {0x11, 0x1F, 0x11}, //  73 'I'
    {0x10, 0x10, 0x0F}, //  74 'J'
    {0x1F, 0x04, 0x1B}, //  75 'K'
    {0x1F, 0x10, 0x10}, //  76 'L'
    {0x1F, 0x02, 0x1F}, //  77 'M'
    {0x1F, 0x06, 0x1F}, //  78 'N'
    {0x0E, 0x11, 0x0E}, //  79 'O'
    {0x1F, 0x05, 0x02}, //  80 'P'
    {0x0E, 0x19, 0x1E}, //  81 'Q'
    {0x1F, 0x0D, 0x16}, //  82 'R'
    {0x12, 0x15, 0x09}, //  83 'S'
    {0x01, 0x1F, 0x01}, //  84 'T'
    {0x0F, 0x10, 0x1F}, //  85 'U'
    {0x0F, 0x10, 0x0F}, //  86 'V'
    {0x1F, 0x08, 0x1F}, //  87 'W'
    {0x1B, 0x04, 0x1B}, //  88 'X'
    {0x03, 0x1C, 0x03}, //  89 'Y'
    {0x19, 0x15, 0x13}, //  90 'Z'
    {0x00, 0x00, 0x00}, //  91 '['
    {0x00, 0x00, 0x00}, //  92 '\'
    {0x00, 0x00, 0x00}, //  93 ']'
    {0x00, 0x00, 0x00}, //  94 '^'
    {0x00, 0x00, 0x00}, //  95 '_'
    {0x00, 0x00, 0x00}, //  96 '`'
    {0x18, 0x14, 0x1C}, //  97 'a'
    {0x1F, 0x14, 0x08}, //  98 'b'
    {0x0C, 0x14, 0x14}, //  99 'c'
    {0x08, 0x14, 0x1F}, // 100 'd'
    {0x0C, 0x1C, 0x14}, // 101 'e'
    {0x04, 0x1E, 0x05}, // 102 'f'
    {0x0C, 0x2A, 0x1E}, // 103 'g'
    {0x1F, 0x04, 0x18}, // 104 'h'
    {0x00, 0x1D, 0x00}, // 105 'i'
    {0x10, 0x10, 0x0D}, // 106 'j'
    {0x1F, 0x08, 0x14}, // 107 'k'
    {0x11, 0x1F, 0x10}, // 108 'l'
    {0x1C, 0x04, 0x1C}, // 109 'm'
    {0x1C, 0x04, 0x18}, // 110 'n'
    {0x0C, 0x14, 0x0C}, // 111 'o'
    {0x3C, 0x14, 0x08}, // 112 'p'
    {0x08, 0x14, 0x3C}, // 113 'q'
    {0x1C, 0x04, 0x04}, // 114 'r'
    {0x14, 0x1C, 0x0C}, // 115 's'
    {0x04, 0x1F, 0x14}, // 116 't'
    {0x0C, 0x10, 0x1C}, // 117 'u'
    {0x1C, 0x10, 0x0C}, // 118 'v'
    {0x1C, 0x10, 0x1C}, // 119 'w'
    {0x14, 0x08, 0x14}, // 120 'x'
    {0x14, 0x18, 0x0C}, // 121 'y'
    {0x1C, 0x14, 0x1C}, // 122 'z'
    {0x00, 0x00, 0x00}, // 123 '{'
    {0x00, 0x00, 0x00}, // 124 '|'
    {0x00, 0x00, 0x00}, // 125 '}'
    {0x00, 0x00, 0x00}, // 126 '~'
    {0x00, 0x00, 0x00}, // 127 'DEL'
};
#endif